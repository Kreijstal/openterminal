...
The measurements are CI output, not repository content: 840 files regenerated
...
corpus the last run saw — the 90 L5 cases, the 14 newer `L0-props` ones and the
195 in `L1-shape`, `L2-content` and `L3-canvas` were all authored after it.
...
`10.0.26100.33158` covers 541 cases and no L5; the corpus now has 840 and does.
...
A further 285 generated cases in `L1-shape`, `L2-content`, `L3-canvas`,
`L5-resources` and `L5-styles` are newer than the last oracle run for the same
reason. They do
