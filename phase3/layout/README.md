...
`L1-shape`, `L2-content` and `L3-canvas` have left this table: they were
measured, and all three are answered by the layout-participation rule and the
`ContentPresenter` alignment default above. `L3-scroll` and `L4-icon` are a
different kind of open from the rest. `L0-props`, `L4-source` and all five L5
